...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			for (int eloops = 0; eloops < trials; eloops ++) {
				EfficientDocument ed = new EfficientDocument(text);
				double efs = ed.getFleschScore();
			}
			long endEfficientTime = System.nanoTime();
			double efficientTime = (endEfficientTime - startEfficientTime) / 100000000.0;
			
			long startBasicTime = System.nanoTime();
			for (int bloops = 0; bloops < trials; bloops ++) {
				BasicDocument bd = new BasicDocument(text);
				double bfs = bd.getFleschScore();
			}
			long endBasicTime = System.nanoTime();
			double basicTime = (endBasicTime - startBasicTime) / 100000000.0;
