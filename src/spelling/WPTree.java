...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(!queue.isEmpty() && !path.contains(word2)) {
			WPTreeNode currNode = ((LinkedList<WPTreeNode>)queue).removeFirst();
			List<String> wordList = nw.distanceOne(currNode.getWord(),true);
			for(String w : wordList) {
				if (!visited.contains(w)) {
					visited.add(w);
					currNode = currNode.addChild(w);
					queue.add(currNode);
					if(w.equals(word2)) {
						path = currNode.buildPathToRoot();
						return path;
					}
				}
			}
